...
class Placement:
    ...
    def rigid(self) -> Dict[Tuple, Tuple]:
    ...
    def affine(self) -> Dict[Tuple, Affine]:
        ...
        unity. This works best if the transformations are relatively
        close to unity to begin with: Rotations greater than 90°
        generally do not lead to good results.
        ...
        for mtch in self.matches:
            ...
            for n in range(2):
                for i in range(2):
                    for j in range(2):
                        M[Aindex(m,n,i), Aindex(m,n,j)] += 2*p[i] * p[j]
                        M[Aindex(m,n,i), Aindex(m_,n,j)] -= 2*p[i] * p_[j]
                    M[Aindex(m,n,i), Xindex(m, n)] += 2 * p[i]
                    M[Aindex(m,n,i), Xindex(m_, n)] -= 2 * p[i]

            # These are the dE/dX terms in A and X
            for n in range(2):
                for j in range(2):
                    M[Xindex(m,n), Aindex(m,n,j)] += 2 * p[j]
                    M[Xindex(m,n), Aindex(m_,n,j)] -= 2 * p_[j]
                M[Xindex(m,n), Xindex(m,n)] += 2
                M[Xindex(m,n), Xindex(m_,n)] -= 2
